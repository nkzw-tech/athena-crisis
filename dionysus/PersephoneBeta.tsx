...
type PersephoneStrategicPlan = Readonly<{
  capturableBuildings: number;
...
// PersephoneBeta is a direct fork of DiomedesEpsilon with conservative sea transport improvements.
// Keep changes local to this file so DionysusAlpha campaign behavior remains stable.
export default class PersephoneBeta extends BaseAI {
  private strategicPlan: PersephoneStrategicPlan | null = null;
  ...
  private getStrategicSummary(map: MapData, currentPlayer: Player) {
    if (!this.strategicPlan || this.strategicPlan.currentPlayer !== currentPlayer.id) {
      this.strategicPlan = createPersephoneStrategicPlan(map, currentPlayer);
    }

    return createPersephoneStrategicSummary(map, currentPlayer, this.strategicPlan);
  }
  ...
  private attack(map: MapData): MapData | null {
    ...
    possibleAttacks = sortPersephonePossibleAttacks(map, currentPlayer, vision, possibleAttacks);
    ...
    while (possibleAttacks.length) {
      ...
      if (dirtyUnits.size) {
        ...
        possibleAttacks = sortPersephonePossibleAttacks(
          currentMap,
          currentPlayer,
          vision,
          possibleAttacks,
  ...
  private createUnit(map: MapData): MapData | null {
    ...
    const getUnitInfos = (building: Building, vector: Vector) => {
      if (!unitInfoMap.has(building.id)) {
        ...
        const unitInfos = includePersephoneStrategicUnitInfos(
    ...
    for (const [from, building] of buildings) {
      ...
      const rankedUnitInfos = hasPersephoneProductionDemand(strategicSummary)
        ? rankPersephoneUnitChoices(currentPlayer, sortedUnitInfos, strategicSummary)
        : sortedUnitInfos;
        ...
        if (
          ...
          combinations.push({
            ...
            weight:
              ...
              (hasPersephoneProductionDemand(strategicSummary)
                ? getPersephoneProductionWeight(unitInfo, currentPlayer, strategicSummary)
                : 0),
          });
        }
      }
  ...
  private move(map: MapData): MapData | null {
    ...
    const clusters = calculateClusters(
      map.size,
      hasPotentialPersephoneTransportDemand(map, currentPlayer)
        ? getPersephoneInterestingVectors(
            map,
...
const createPersephoneStrategicPlan = (
  map: MapData,
  currentPlayer: Player,
): PersephoneStrategicPlan => {
  const ownUnits = [...map.units.filter((unit) => map.matchesPlayer(unit, currentPlayer)).values()];
  if (!hasPotentialPersephoneTransportDemand(map, currentPlayer, ownUnits)) {
    return {
  ...
  const hasLoadedNavalTransport = ownUnits.some(isLoadedNavalTransport);
  const transportTargets = sortBy(
    capturableBuildings.filter(
      (vector) =>
        isRemotePersephoneTransportTarget(map, currentPlayer, ownUnits, vector) &&
        (!hasLoadedNavalTransport ||
          isValuablePersephoneTransportTarget(map, currentPlayer, vector)),
    ),
  ...
  return {
    ...
    currentPlayer: currentPlayer.id,
    transportStagingReady: hasPersephoneTransportStaging(map, currentPlayer),
    transportTargets,
  };
};

const hasPotentialPersephoneTransportDemand = (
  map: MapData,
  currentPlayer: Player,
  ...
  (map.round <= 8 &&
    ...
    hasPersephoneTransportStaging(map, currentPlayer)) ||
  (map.round <= 12 && ownUnits.length <= 18 && ownUnits.some(isLoadedNavalTransport));

const createPersephoneStrategicSummary = (
  map: MapData,
  currentPlayer: Player,
...
const hasPersephoneTransportStaging = (map: MapData, currentPlayer: Player) => {
...
const isRemotePersephoneTransportTarget = (
  map: MapData,
  currentPlayer: Player,
...
const isValuablePersephoneTransportTarget = (
  map: MapData,
  currentPlayer: Player,
  target: Vector,
) => {
  const building = map.buildings.get(target);
  if (!building || map.matchesTeam(building, currentPlayer)) {
    return false;
  }

  return (
    building.info.isHQ() ||
    building.info.canBuildUnits() ||
    building.info.configuration.funds >= 200 ||
    getBuildingWeight(building.info) >= 15
  );
};

const isLoadedNavalTransport = (unit: Unit) =>
  unit.info.canTransportUnits() && isNavalUnit(unit) && unit.isTransportingUnits();

const getPersephoneInterestingVectors = (
  map: MapData,
  ...
  if (isLoadedNavalTransport(unit) && strategicSummary.transportTargets.length) {
...
const includePersephoneStrategicUnitInfos = (
...
const hasPersephoneProductionDemand = (strategicSummary: PersephoneStrategicSummary) =>
  strategicSummary.airCounterDemand ||
  strategicSummary.transportDemand > strategicSummary.ownTransportCapacity;

const rankPersephoneUnitChoices = (
  currentPlayer: Player,
  unitInfos: ReadonlyArray<UnitInfo>,
  strategicSummary: PersephoneStrategicSummary,
) =>
  sortBy(
    unitInfos.map((unitInfo, index) => ({
      unitInfo,
      weight: index * 4 + getPersephoneProductionWeight(unitInfo, currentPlayer, strategicSummary),
    })),
    ({ weight }) => weight,
  ).map(({ unitInfo }) => unitInfo);

const getPersephoneProductionWeight = (
  unitInfo: UnitInfo,
  currentPlayer: Player,
  strategicSummary: PersephoneStrategicSummary,
) => {
...
const getPersephoneAttackOutcomeMultiplier = (map: MapData, currentPlayer: Player) =>
...
const sortPersephonePossibleAttacks = (
  map: MapData,
  currentPlayer: Player,
  vision: VisionT,
  ...
  sortBy(
    possibleAttacks.map((attack) => ({
      attack,
      weight:
        getPersephoneAttackWeight(map, currentPlayer, attack) +
        getAttackOutcomeWeight(map, currentPlayer, vision, attack) *
          getPersephoneAttackOutcomeMultiplier(map, currentPlayer),
    })),
    ({ weight }) => weight,
  ).map(({ attack }) => attack);

const getPersephoneAttackWeight = (map: MapData, currentPlayer: Player, item: PossibleAttack) => {

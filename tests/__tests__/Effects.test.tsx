...
test('triggers effects for actions generated by effects', async () => {
  const vecA = vec(1, 1);
  const vecB = vec(3, 1);
  const vecC = vec(3, 3);
  const vecD = vec(5, 5);
  ...
  const effects: Effects = new Map([
    [
      ...
      new Set([
        {
          actions: [Spawn([[3, 3, Pioneer.create(1).toJSON()]])],
          conditions: [{ type: 'OptionalObjective', value: 1 }],
        },
      ]),
    ],
  ]);
  ...
  expect(snapshotEncodedActionResponse(gameActionResponse))
    ...
    Spawn { units: [3,3 → Pioneer { id: 1, health: 100, player: 1, fuel: 40, name: 'Sam' }], teams: null }

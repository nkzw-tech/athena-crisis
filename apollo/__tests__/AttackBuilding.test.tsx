...
test('units do not disappear when a building is attacked and there is no counter attack', async () => {
  ...
  const [, mapState1] = execute(
    map,
    vision,
    AttackBuildingAction(vec(2, 1), to),
  )!;
  const [, mapState2] = execute(
    mapState1,
    vision,
    AttackBuildingAction(vec(1, 2), to),
  )!;
  expect(mapState1.units.get(to)).toEqual(Pioneer.create(2));
  expect(mapState2.units.get(to)).toBeUndefined();

  const to2 = vec(5, 5);
  const [, mapState3] = execute(
    map,
    vision,
    AttackBuildingAction(vec(4, 5), to2),
  )!;
  const [, mapState4] = execute(
    mapState3,
    vision,
    AttackBuildingAction(vec(5, 4), to2),
  )!;

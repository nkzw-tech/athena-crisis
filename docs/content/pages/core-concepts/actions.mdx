...
function sleep(map: MapData, { from }: SleepAction) {
  ...
  return unit and map.isCurrentPlayer(unit) && !unit.isSleeping()
...
For convenience, we'll also add an [`ActionMutator`](https://github.com/nkzw-tech/athena-crisis/blob/main/apollo/action-mutators/ActionMutators.tsx). These are simple functions to avoid repetition when executing actions against game state, as is often the case in tests.

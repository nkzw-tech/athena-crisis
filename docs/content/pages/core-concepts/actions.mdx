...
```tsx
const response = executeGameActions(map, [MoveAction(from, to), SleepAction(to), EndTurnAction()]);
```
...
```tsx
export async function sleepAction(actions: Actions, state: State): Promise<State> {
  ...
  const [remoteAction, newMap, actionResponse] = actions.action(
    state,
    SleepUnitAction(selectedPosition),
  );
  if (actionResponse.type !== 'Sleep') {
    return actions.update(null);
  }
  ...
  return handleRemoteAction(actions, remoteAction, 'Sleep');
}
```
...
In the above example, `action(state, SleepUnitAction(from))` returns the remote request together with the locally computed map and ActionResponse. The client can display that result immediately while the server processes the same action.

Keep the behavior locked until the primary animation has completed. For actions where a rejection would make the optimistic state unsafe to continue from, pass the remote request to `handleRemoteAction`, which validates the server's own ActionResponse, processes any additional effect responses, and finally restores the behavior.

Actions that are safe to continue optimistically can restore the behavior as soon as their primary animation completes and reconcile the remote response afterward with `restoreBehavior: false`. This retains the fluid local interaction while still validating the server response and processing every effect. A rejected or contradictory response uses the normal error path to reload authoritative state.

The server always has the final say on game state. In both variants, process the remote response only after the primary animation completes so effects cannot race the animation that triggered them.
